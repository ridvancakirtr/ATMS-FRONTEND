...
export default {
  ...
  watch:{
    currentPage(value){
      if(this.filter!=null){
        this.fetchUsers({page:value,limit:this.perPage,search:this.filter})
      }else{
        this.fetchUsers({page:value,limit:this.perPage,search:""})
      }
      
    },
    perPage(value){
      this.fetchUsers({page:this.currentPage,limit:value,search:""})
    },
    filter(value){
      this.fetchUsers({page:"",limit:this.perPage,search:value})
      this.currentPage=1
    }
  },
  computed: {
    rows() {
      if(this.$store.state.user.isLoad){
        return this.$store.state.user.users.totalPages
      }
      return 1
    },
    users(){
  ...
  mounted() {
    this.fetchUsers({page:this.currentPage,limit:this.perPage,search:""})
  },
  methods: {
    ...userMethod
  },
          ...
          <div class="card-body">
            ...
            <div class="table-responsive mb-0">
              <b-table
                ...
                :current-page="1"
                :filter="filter"
                :busy="!this.$store.state.user.isLoad"
              >
              <template #cell(phone)="row">
                {{ row.value.formattedNumber }}
              </template>
              <template #cell(role)="row">
                {{ row.item.role ? 'Operatör' : 'Yönetici' }}
              </template>
              ...
              <div class="col">
                <div class="dataTables_paginate paging_simple_numbers float-end">
                  <ul class="pagination pagination-rounded mb-0">
                    ...
                    <b-pagination
                      v-model="currentPage"
                      :total-rows="this.$store.state.user.users.totalDocs"
                      :per-page="perPage"
                    ></b-pagination>
                  </ul>
                </div>
